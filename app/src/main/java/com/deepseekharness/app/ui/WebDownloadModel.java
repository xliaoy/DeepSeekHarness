...
public final class WebDownloadModel extends AndroidViewModel {
    ...
    public synchronized void restoreState(Bundle saved) {
        ...
        if (id.matches("[0-9a-fA-F-]{36}\\.part")) {
            ...
            if (candidate.isFile() && candidate.length()==saved.getLong("web-file-size",-1)) {
                ...
                state.setValue(new State("ready","下载完成，选择保存位置",candidate.length(),candidate.length()));return;
            }
        }
        if (!id.isEmpty() || saved.getBoolean("web-file-interrupted")) state.setValue(new State("error","下载被系统中断，请重新导出",0,-1));
    }
    public synchronized boolean download(String base, String url, String cookie, String filename,
        ...
        if (!(supplied != null ? WebPreviewPolicy.pageDownload(base,url) : WebPreviewPolicy.sameService(base, url))) {
            close(supplied); state.setValue(new State("error", "此下载不是当前本机服务提供的文件",0,-1)); return false;
        }
        busy = true; cancelled = false; name = WebTransferPolicy.fileName(filename);
        state.setValue(new State("downloading", "正在下载 " + name, 0, expected));
        new Thread(() -> {
            File file = null;
            State result;
            try (RuntimeTasks ignored = RuntimeTasks.begin()) {
                File dir = new File(getApplication().getCacheDir(), "web-downloads");
                if (!dir.isDirectory() && !dir.mkdirs()) throw new IOException("无法创建下载缓存");
                ...
                if (supplied != null) input = supplied;
                    ...
                    for (int redirects = 0; ; redirects++) {
                        if (!WebPreviewPolicy.sameService(base, next)) throw new IOException("下载跳转到了其他服务");
                        checkCancelled();
                        ...
                        if (status >= 300 && status <= 399 && redirects < 3) {
                            ...
                            if (location == null) throw new IOException("下载跳转地址缺失");
                ...
                if (total > 0 && dir.getUsableSpace() < total + 16L*1024*1024) throw new IOException("存储空间不足");
                long bytes = 0, last = 0;
                try (InputStream in = input; FileOutputStream out = new FileOutputStream(file)) {
                    if (in == null) throw new IOException("下载响应没有文件内容");
                    ...
                    while ((n = in.read(buffer)) != -1) {
                        ...
                        if (android.os.SystemClock.elapsedRealtime() - last > 200) {
                            ...
                            state.postValue(new State("downloading", "正在下载 " + name, bytes, total));
                        }
                    }
                    out.getFD().sync();
                }
                ...
                result = new State("ready", "下载完成，选择保存位置", bytes, bytes);
    ...
    public synchronized void save(Uri uri) {
        pickerOpen = false;
        if (uri == null) { discard(); state.setValue(new State("idle", "已取消保存",0,-1)); return; }
        if (busy) return;
        if (staged == null) {
            try { android.provider.DocumentsContract.deleteDocument(getApplication().getContentResolver(),uri); } catch(Exception ignored) { }
            state.setValue(new State("error","下载缓存已丢失，请重新导出",0,-1)); return;
        }
        busy = true; cancelled = false;
        final File file = staged;
        state.setValue(new State("saving", "正在保存并校验文件", 0, file.length()));
        new Thread(() -> {
            State status;
            try (RuntimeTasks ignored = RuntimeTasks.begin()) {
                ...
                try (InputStream in = getApplication().getContentResolver().openInputStream(uri)) {
                    if (in == null) throw new IOException("保存后无法读取校验");
                    byte[] buffer = new byte[65536]; int n;
                    while ((n = in.read(buffer)) != -1) { checkCancelled(); count += n; result.update(buffer,0,n); }
                }
                if (count != file.length() || !MessageDigest.isEqual(source.digest(), result.digest())) throw new IOException("保存校验失败，请更换位置重试");
                lastSavedUri = uri; discard(); status = new State("done", "已保存：" + name, count, count);
    ...
    public void retrySave() { if (!busy && staged != null) state.setValue(new State("ready","选择保存位置",staged.length(),staged.length())); }
    private void checkCancelled() throws IOException { if (cancelled) throw new IOException("已取消"); }
    private State failure(Exception error) { return new State("error", cancelled ? "已取消下载或保存" : "文件处理失败：" + error.getMessage(),0,-1); }

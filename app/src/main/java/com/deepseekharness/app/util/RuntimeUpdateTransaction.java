...
public final class RuntimeUpdateTransaction {
    ...
    public static RuntimeUpdateTransaction create(File files) throws IOException {
        if (pending(files) != null) throw new IOException("先恢复上次运行时更新");
        File home = new File(files.getCanonicalFile(), "runtime-updates");
        if (!home.getAbsoluteFile().equals(home.getCanonicalFile())) throw new IOException("运行时维护目录不安全");
        if (!home.isDirectory() && !home.mkdirs()) throw new IOException("无法建立运行时维护目录");
        File entry = new File(home, UUID.randomUUID().toString());
        if (!entry.mkdir()) throw new IOException("无法建立运行时事务");
    ...
    public static RuntimeUpdateTransaction pending(File files) throws IOException {
        ...
        if (entries == null) { if (home.exists()) throw new IOException("无法读取运行时维护日志"); return null; }
        ...
        for (File entry : entries) if (entry.getName().matches("[a-f0-9-]{36}")) {
            ...
            if (new File(entry, "intent.properties").isFile() && !item.finished()) {
                if (pending != null) throw new IOException("多份运行时维护未完成，已停止切换");
                pending = item;
            }
        }
        return pending;
    ...
    private void local(File file) throws IOException {
        ...
        if (!absolute.getCanonicalFile().equals(absolute) || !absolute.getPath().startsWith(files.getPath() + File.separator))
            throw new IOException("运行时维护路径不安全：" + file.getName());
    }
    ...
    private static boolean present(File file) throws IOException {
        ...
        if (names == null) { if (parent.exists()) throw new IOException("无法读取运行时路径状态"); return false; }
    ...
    private void mark(String name, String text) throws IOException {
        ...
        if (target.exists()) throw new IOException("运行时阶段已存在：" + name);
        ...
        if (!tmp.renameTo(target)) throw new IOException("无法提交运行时维护阶段");
    }
    public void begin(List<String> paths) throws IOException {
        if (paths.isEmpty() || paths.size() > 2048 || new HashSet<>(paths).size() != paths.size()) throw new IOException("运行时更新清单无效");
        StringBuilder intent = new StringBuilder("count=" + paths.size() + "\n");
        for (int i = 0; i < paths.size(); i++) {
            String path = paths.get(i);
            if (!ManagedRuntimeLayout.allowed(path)) throw new IOException("禁止更新用户数据路径");
            File target = new File(files, path), source = new File(stage(), path);
            parent(target); parent(source);
            if (!present(source)) throw new IOException("新运行时缺少文件：" + path);
    ...
    private List<String> paths(Properties intent) throws IOException {
        ...
        catch (NumberFormatException error) { throw new IOException("运行时维护日志无效", error); }
        if (count <= 0 || count > 2048) throw new IOException("运行时维护清单数量无效");
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!ManagedRuntimeLayout.allowed(path) || result.contains(path) || !("true".equals(had) || "false".equals(had)))
                throw new IOException("运行时维护清单无效");
            result.add(path);
        }
        return result;
    }
    private void move(File from, File to) throws IOException {
        ...
        if (!to.getParentFile().isDirectory() && !to.getParentFile().mkdirs()) throw new IOException("无法建立切换目录");
        if (present(to) || !from.renameTo(to)) throw new IOException("无法切换运行时：" + from.getName());
    }
    public void replace() throws IOException {
        if (finished()) throw new IOException("运行时事务已结束");
    ...
    public void commit() throws IOException {
        for (String path : paths(intent())) {
            ...
            if (!present(target) || present(new File(stage(), path))) throw new IOException("运行时文件尚未切换完整");
        }
        mark("committed", "ok\n");
    }
    public void rollback() throws IOException {
        ...
        for (int i = paths.size() - 1; i >= 0; i--) {
            ...
            } else if (!present(target)) throw new IOException("旧运行时位置不明，已保留全部现场");
        }
        mark("rolled-back", "ok\n");
    }

...
// apply 挂载三块表面：侧边栏入口按钮 + 浮层抽屉面板 + 宿主设置弹窗的一级设置项。
...
/** 面板入口显隐状态（设置页 panel.enabled 开关与侧边栏入口按钮共享）。 */
const panelState = { enabled: true }
const panelListeners = new Set()
/** 更新入口显隐并通知侧边栏按钮；把任意值归一为布尔。 */
function setPanelEnabled(visible) {
  const next = visible !== false
  if (panelState.enabled !== next) {
    panelState.enabled = next
    for (const listener of panelListeners) listener()
  }
}
/** 侧边栏入口按钮订阅显隐变化（返回退订函数）。 */
function subscribePanelEnabled(listener) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.error === undefined) {
        const enabled = data.panel?.enabled !== false
        setPanelEnabled(enabled)
        return { enabled, language: data.language ?? 'en' }
      }
    }
  } catch {
    // 探测失败：保持默认开启。
...
[data-mobile-nav="mem-entry"] { display: inline-flex; align-items: center; justify-content: center; gap: 6px; width: 100%; height: 34px; padding: 0 12px; border: 1px solid var(--dsw-alias-border-l1, rgba(0, 0, 0, .12)); border-radius: 12px; background: transparent; color: var(--dsw-alias-label-primary, inherit); font-family: inherit; font-size: 13px; line-height: 20px; cursor: pointer; }
[data-mobile-nav="mem-entry"]:hover { background: var(--dsw-alias-interactive-bg-hover, #2c2c33); }
...
`
  ...
  /** 打开/关闭抽屉（侧边栏入口按钮共用；打开时刷新一次）。 */
  const toggleDrawer = () => {
    drawer.style.display = drawer.style.display === 'none' ? 'flex' : 'none'
    if (drawer.style.display !== 'none') void refresh()
  }
  /** 侧边栏入口按钮通过这两个全局函数操作面板。 */
  window.__dshMementoTogglePanel = toggleDrawer
  window.__dshMementoRefreshPanel = () => void refresh()
...
/**
 * 侧边栏入口按钮：注入 sidebar.footer.action（order 3，位于 dsh-balance-panel
 * 上方），样式与 dsh-web-mobile 的"文件浏览/会话日志"按钮一致（一行独立按钮）。
 * 点击打开/关闭记忆抽屉；面板尚未安装（panel.enabled=false 后首次开启）时先
 * bootPanel 再打开。显隐跟随设置页 panel.enabled 开关。
 */
function MementoSidebarButton() {
  const [visible, setVisible] = react.useState(panelState.enabled)
  react.useEffect(() => subscribePanelEnabled(setVisible), [])
  if (!visible) return null
  const open = () => {
    const toggle = window.__dshMementoTogglePanel
    if (typeof toggle === 'function') { toggle(); return }
    void bootPanel().then(() => {
      const retry = window.__dshMementoTogglePanel
      if (typeof retry === 'function') retry()
    })
  }
  return jsx('button', {
    type: 'button',
    'data-mobile-nav': 'mem-entry',
    title: '记忆面板（dsh-memento）',
    onClick: open,
  },
...
`
      ...
      /** 控制器：scope → 暂存表单 → 设置页快照。保存落盘后同步侧边栏入口按钮显隐。 */
        ...
        constructor(scope) {
          ...
          this.form = new CardForm(scope, (tops) => {
            ...
            if (panel !== null && typeof panel === 'object' && 'enabled' in panel) {
              setPanelEnabled(panel.enabled === true)
            }
          })
      ...
      function apply(ctx) {
        ...
        ctx.effect(() => ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
          ...
          id: 'dsh-memento',
          order: 3,
        }, () => jsx(MementoSidebarButton, null))), 'dsh-memento: sidebar entry')
      }
